...
def register_api_routes(app):
    ...
    def api_register():
        ...
        if not username or not email or not password: return jsonify({"msg": "Faltan datos obligatorios"}), 400
        if UserRepository().get_by_email(email): return jsonify({"msg":"Email ya existe"}), 400
    ...
    def api_login():
        ...
        if not usuario or not usuario.credentials.check_password(password): return jsonify({"msg":"Credenciales incorrectas"}), 401
    ...
    def get_stats():
        ...
        stats = {"total_posts": len(PostRepository().get_all()), "total_comments": len(ComentarioRepository().get_all()), "total_users": len(UserRepository().get_all())}
        if current_user_role == 'admin': stats["posts_last_week"] = PostRepository().count_posts_last_week()
